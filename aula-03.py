...
llm_triagem = ChatGoogleGenerativeAI(
    model="gemini-2.5-flash", temperatura=0.0, google_api_key=GEMINI_API_KEY
)
...
def decidir_pos_auto_resolver(state: AgentState) -> str:
    ...
    if any(k in state_da_pergunta for k in KEYWORDS_ABRIR_TICKET):
        print(
            "Rag falhou, mas foram encontradas keywords de abertura de ticket. Abrindo..."
        )
        return "chamado"
...
for msg_test in testes:
    ...
    print(
        f"DECISÃO: {triag.get('decisao')} | URGÊNCIA: {triag.get('urgencia')} | AÇÃO FINAL: {resposta_final.get('acao_final')}"
    )
